import { costOf, costDetailed, priceFor } from "./lib/pricing.js";
...
ok("SAFE: unknown model cached tokens billed at FULL input (never under-count)", near(costDetailed("mystery-model-9",{inTokens:1000,cachedInTokens:1000}),0.005));

console.log("PRICING — xAI / GROK:");
ok("grok-4.5 1000in/500out = $0.005", near(costOf("grok-4.5",1000,500),0.005));
ok("grok-4.3 priced right ($0.00375 for 1k/1k)", near(costOf("grok-4.3",1000,1000),0.00375));
ok("grok-4.20 variant resolves via prefix", priceFor("grok-4.20-0309-reasoning").in===1.25 && priceFor("grok-4.20-multi-agent-0309").out===2.50);
...
ok("groq moonshotai/kimi-k2-instruct-0905 → kimi-k2 row", priceFor("moonshotai/kimi-k2-instruct-0905").out===3.00);
...
ok("mistral-large-latest = $2/$6", priceFor("mistral-large-latest").in===2.00 && priceFor("mistral-large-latest").out===6.00);
ok("mistral-small-3.1 = $0.10/$0.30", priceFor("mistral-small-3.1").in===0.10);
ok("codestral prices on its own row", priceFor("codestral-2501").in===0.30);
ok("unknown mistral model → large catch-all (safe-higher)", priceFor("mistral-medium-3.5").in===2.00);
...
ok("OpenRouter dotted Claude name → claude catch-all (sonnet rate)", priceFor("anthropic/claude-3.5-sonnet").in===3.00 && priceFor("anthropic/claude-3.5-sonnet").out===15.00);
